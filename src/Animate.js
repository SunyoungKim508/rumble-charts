...
export default class Animate extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        const interpolate = d3Interpolate(
            _.pick(prevState, prevProps.interpolateProps),
            _.pick(
                _.assign({}, this.props, normalizeSeries(this.props)),
                prevProps.interpolateProps
            )
        )

        const { duration, onStart, onEnd, logFPS } = prevProps
        let { ease } = prevProps
    ...
    render() {
        ...
        return (
            <g className={props.className}>
                {proxyChildren(props.children, _.omitBy(state, _.isUndefined), {
